...
function fish_right_prompt
    ...
    # AWS profile (dim yellow, only if set)
    ...
    # Bail early if git not available or not inside repo
    if not command -sq git
        return
    end
    ...
    # Porcelain status
    set -l porcelain (command git status --porcelain --ignore-submodules=dirty 2>/dev/null | string sub -l2)

    set -l added     (string match -qr '[ACMRT] ' $porcelain; and echo 1)
    set -l deleted   (string match -qr '[ ACMRT]D' $porcelain; and echo 1)
    set -l modified  (string match -qr 'M$' $porcelain; and echo 1)
    set -l renamed   (string match -qe R $porcelain; and echo 1)
    set -l unmerged  (string match -qr 'AA|DD|U' $porcelain; and echo 1)
    set -l untracked (string match -qe '\?\?' $porcelain; and echo 1)

    # Branch name
    if test -n "$branch"
        if test $branch_detached -eq 1
            set_color brmagenta
        else
            set_color brgreen
        end
